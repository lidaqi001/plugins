...
var timerMapPool = &sync.Pool{New: func() interface{} {
	return map[uint32]*Timer{}
}}
...
func NewTimeWheel(name string, interval int64, scales int, maxCap int) *TimeWheel {
	...
	//初始化map
	for i := 0; i < scales; i++ {
		tmp := timerMapPool.Get().(map[uint32]*Timer) // 申请变量
		tmp = map[uint32]*Timer{}                     // 重置变量
		tw.timerQueue[i] = tmp
	}
...
func (tw *TimeWheel) run() {
	for {
		...
		tmp := timerMapPool.Get().(map[uint32]*Timer) // 申请变量
		tmp = map[uint32]*Timer{}                     // 重置变量
		tw.timerQueue[tw.curIndex] = tmp
		for tID, timer := range curTimers {
			//这里属于时间轮自动转动，forceNext设置为true
			tw.addTimer(tID, timer, true)
		}
		timerMapPool.Put(curTimers) // 释放变量

		//取出下一个刻度 挂载的全部定时器 进行重新添加 (为了安全起见,待考慮)
		nextTimers := tw.timerQueue[(tw.curIndex+1)%tw.scales]
		tmp2 := timerMapPool.Get().(map[uint32]*Timer) // 申请变量
		tmp2 = map[uint32]*Timer{}                     // 重置变量
		tw.timerQueue[(tw.curIndex+1)%tw.scales] = tmp2
		for tID, timer := range nextTimers {
			tw.addTimer(tID, timer, true)
		}
		timerMapPool.Put(nextTimers) // 释放变量
...
func (tw *TimeWheel) GetTimerWithIn(duration time.Duration) map[uint32]*Timer {
	...
	defer leaftw.Unlock()
	//返回的Timer集合
	timerList := timerMapPool.Get().(map[uint32]*Timer) // 申请变量
	defer timerMapPool.Put(timerList)                   // 释放变量
	timerList = map[uint32]*Timer{}                     // 重置
